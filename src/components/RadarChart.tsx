...
import { RadarChartProps } from "@/lib/types";
...
const RadarChart: React.FC<RadarChartProps> = ({ usersData, focusedUser }) => {
  const [activeDataset, setActiveDataset] = useState<number | null>(0);
  ...
  const datasets = usersData.slice(0, 5).map((value, index) => {
    ...
    return {
      ...
      backgroundColor:
        activeDataset === index ? color?.activeBackground : color?.background,
  ...
  datasets.sort((a, b) => Number(b.isActive) - Number(a.isActive));
